...
def parse_datetime(date_str: str, time_str: str, timezone_str: str) -> Tuple[float, datetime]:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid date/time format: {str(e)}. Expected format: 'DD-MM-YYYY HH:MM AM/PM'"
        )
...
def calculate_planetary_positions(jd: float, latitude: float, longitude: float) -> Dict[str, Any]:
    ...
    try:
        ...
        # ✅ NEW: Mars in same sidereal system
        mars = swe.calc_ut(jd, swe.MARS, iflag)
        mars_long = mars[0][0]
...
def generate_kundli(data: KundliRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "planetary_longitudes": {
            ...
            # ✅ NEW: Mars longitude included for compatibility/manglik checks
            "mars": round(positions["mars_long"], 6),
        },
        "metadata": {
